...
void process(Mat & frame)
{
	...
	///imshow("result of WT", Dst);
	///imshow("result of IWT", Filtered);
	//---------------------------------------------------- 
	...
	///imshow("result of normalize WT", Dst);

	//minMaxLoc(Filtered, &m, &M);
	//if ((M - m) > 0) { Filtered = Filtered * (1.0 / (M - m)) - m / (M - m); }
	///normalize(Filtered, Filtered, 0, 1, 32);
	///imshow("result of normalize IWT", Filtered);
}